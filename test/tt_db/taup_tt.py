...
build_taup_model("kumamoto.nd")
model = TauPyModel(model="kumamoto")

with open("ttdb.txt", "w") as f:
    #f.write("dist dep tp ts tp_slowness ts_slowness tp_hslowness ts_hslowness p_elvecorr s_elvecorr\n")
    for dep in range(0,31,2): # search 0 - 20 km in depth
        for dist in range(1,151,1): # search 0.02 - 3.0 degree in horizontal 
            dist = dist*0.02
            print(dep,dist)
